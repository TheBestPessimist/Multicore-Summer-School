...
private: System::Void button3_Click(System::Object^  sender, System::EventArgs^  e) 
		 {
			...
			typedef void (CALLBACK* LPFNDLLFUNC1)(int, int, int*, int*);

			int tbv=trackBar1->Value;
			int W = poza1->Width;
			int *PRx=(int*)malloc(tbv*sizeof(int)),
				*PRy=(int*)malloc(tbv*sizeof(int));

	HINSTANCE hDLL;               // Handle to DLL
	LPFNDLLFUNC1 lpfnDllFunc1;    // Function pointer

	hDLL = LoadLibrary(L"F:/hpc/cuda_dll2.dll");
	lpfnDllFunc1 = (LPFNDLLFUNC1)GetProcAddress(hDLL, "randCallF");
	lpfnDllFunc1(W,tbv, PRx, PRy);
			 ...
			 for (int j = 2; j <tbv-2; j++)
			 {
				// Point punctCurent = puncteRandom[j];
				  Point punctCurent = Point(PRx[j], PRy[j]);
...
QueryPerformanceCounter(&startTime);
			 ...
			 int W=poza1->Width;
			 int *DSx=(int*)malloc(W*sizeof(int)),
				 *DSy=(int*)malloc(W*sizeof(int)),
				 *DPx=(int*)malloc(W*sizeof(int)),
				 *DPy=(int*)malloc(W*sizeof(int)),
				 *DVx=(int*)malloc(W*sizeof(int)),
				 *DVy=(int*)malloc(W*sizeof(int)),
				 *DOx=(int*)malloc(W*sizeof(int)),
				 *DOy=(int*)malloc(W*sizeof(int)),
				 *k1=(int*)malloc(sizeof(int)),
				 *k2=(int*)malloc(sizeof(int)),
				 *k3=(int*)malloc(sizeof(int)),
				 *k4=(int*)malloc(sizeof(int));
			 
			 typedef void (CALLBACK* LPFNDLLFUNC1)(int *,int *,int *,int *, int *, int *, int *, int*, int , int*, int*, int*, int*);

	HINSTANCE hDLL;               // Handle to DLL
	LPFNDLLFUNC1 pixelStarCallF;    // Function pointer

	hDLL = LoadLibrary(L".././hpc/cuda_dll2.dll");		// moved into this folder (check /hpc)
	pixelStarCallF = (LPFNDLLFUNC1)GetProcAddress(hDLL, "pixelStarCallF");
	/*if(hDLL) 
		MessageBox::Show("plm", "My Application", MessageBoxButtons::YesNo, MessageBoxIcon::Question);*/
	pixelStarCallF(DPx, DPy, DSx,DSy, DVx,DVy, DOx, DOy, W, k1, k2, k3, k4);
    char Char_Array[50];
	sprintf(Char_Array,"%d",DSy[2]);  
	String^ t=gcnew String(Char_Array);
	//MessageBox::Show(t, "My Application", MessageBoxButtons::YesNo, MessageBoxIcon::Question);
	//return;
			for (int j = 0; j <(W-2); j++)
			 {		
					 //Point punctCurent = puncteStarDP[j];
					 Point punctCurent = Point(DPx[j], DPy[j]);
					 Color pixelC1 = poza1->GetPixel(punctCurent.X, punctCurent.Y);
					 Color pixelC2 = poza2->GetPixel(punctCurent.X, punctCurent.Y);		
			 ...
			 for (int j = 2; j <(W-2); j++)
				 {
					 ...
					 Point punctCurent1 = Point(DSx[j], DSy[j]);
			...
			for (int j = 0; j <(W-2); j++)
				 {
					 ...
					 Point punctCurent2 = Point(DVx[j], DVy[j]);
			...
			for (int j = 0; j <(W-2); j++)
				 {
					 ...
					 Point punctCurent3 = Point(DOx[j], DOy[j]);
